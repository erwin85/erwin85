...
if (typeof (SBHandler) == 'undefined' && enableSBHandler) {
 
    var SBUtils =
    {
        // userIsInGroup (from Commons:MediaWiki:Utilities.js)
        userIsInGroup : function (group, userGroups)
        {
            if (userGroups) {
    ...
    /**** Enable the whole shebang only for (global) sysops or stewards. */
    if (SBUtils.userIsInGroup('sysop', wgUserGroups) || SBUtils.userIsInGroup('steward', wgGlobalGroups) || SBUtils.userIsInGroup('Global_sysops', wgGlobalGroups)) {
        var SBHandler =
        {
