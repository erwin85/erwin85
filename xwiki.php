...
function formatRow($row, $inreport, $title, $domain, $wiki)
{
    ...
    return "\n" . '<li>' . ($inreport ? '<strong>(R)</strong> ' : '') . formatDate($row['rev_timestamp'], 'en') . ' <i>by <a href = "//' . $domain . '/wiki/User:' . $row['rev_user_text'] . '">' . $row['rev_user_text'] . '</a></i> (' . number_format($row['editcount']) . ' <a href="//' . $domain . '/wiki/Special:Contributions/' . $row['rev_user_text'] . '" title="Contributions">edits</a>' . ($row['rights'] ? '; ' . $row['rights'] : '') . ($grights ? '; ' . $grights : '') . ') (<a href="//' . $domain . '/w/index.php?title=' .  rawurlencode($title) . '&amp;diff=prev&amp;oldid=' . $row['rev_id'] . '" title="' . $title . '">diff</a>)' . ($row['rev_minor_edit'] == '1' ? ' <span class="minor">m</span>' : '') . ' <a href="//' . $domain . '/wiki/' . $title . '" title="' . $title . '">' . str_replace('_', ' ', $title) . '</a> ' . $comment . ($row['page_latest'] == $row['rev_id'] ? ' <strong> (top)</strong>' : '') . ' (' . ($row['page_latest'] == $row['rev_id'] ? '<a href="//' . $domain . '/w/index.php?title=' .  rawurlencode($title) . '&amp;diff=prev&amp;oldid=' . $row['rev_id'] . '&amp;diffonly=yes&amp;xwikirollback" title="' . $title . '">rollback</a> | ' : '' ) . '<a href="//' . $domain . '/w/index.php?title=' . rawurlencode($title) . '&amp;action=edit&amp;undo=' . $row['rev_id'] . '" title="' .  $title . '">undo</a>)</li>';
}
        ...
        if ($db->status[$cluster][0] == 'ERRO' || $db->status[$cluster][0] == 'DOWN') {
            ...
            if($count) {
                ...
                foreach ($lpages as $page=>$link) {
                    ...
                    $removeurl = '//' . $domain . '/w/index.php?title=' .rawurlencode($page) . '&amp;action=edit&amp;linkmodified=yes&amp;remspam=' . $remspam . '&amp;report=' . rawurlencode($report);
                    echo '<li><a href="//' . $domain . '/wiki/' . $page . '" title="' . $page . '">' . str_replace('_', ' ', $page) . '</a> links to <a href="' . $link . '">' . $link . '</a> <span class="removespam" target="_blank"> (<a href="' . $removeurl . '" class="removelink">rem</a>)</span>.</li>';
                    $i++;
                }
                echo '</ul></div>';
            }
            ...
            foreach ($pages as $title=>$page) {
                ...
                $removeurl = '//' . $domain . '/w/index.php?title=' .rawurlencode($title) . '&amp;action=edit&amp;linkmodified=yes&amp;remspam=' . $remspam . '&amp;report=' . rawurlencode($report);
                echo '<li><b>' . str_replace('_', ' ', $title) . '</b> ' . (array_key_exists($title, $lpages) ? '<strong>(L)</strong> ' : '') . '( <a href="//' . $domain . '/w/index.php?title=' . rawurlencode($title) . '&amp;action=history" title="' .  $title . '">hist</a> | <a href="//' . $domain . '/w/index.php?title=' . rawurlencode($title) . '&amp;action=edit" title="' .  $title . '">edit</a>' . (array_key_exists($title, $lpages) ? ' <span class="removespam"> | <a href="' . $removeurl . '" class="removelink" target="_blank">rem</a></span>' : '') . ' ) (Reported edits: ';
                foreach($urls as $url) {
